...
function zoomCategoryScale(scale, zoom, center, zoomOptions) {
	...
	var scaleStep = Math.floor(zoomOptions.step);
	...
	if(isNaN(scaleStep))
		scaleStep = Math.floor((maxIndex - minIndex) /6);
	
	scaleStep = scaleStep == 0 ? scaleStep = 6 : scaleStep;
	...
	if (Math.abs(zoomNS.zoomCumulativeDelta) > sensitivity) {
		if (zoomNS.zoomCumulativeDelta < 0) {
			if (centerPointer >= chartCenter) {
				if (minIndex <= 0) {
					maxIndex = Math.min(lastLabelIndex, maxIndex + scaleStep);
				} else {
					minIndex = Math.max(0, minIndex - scaleStep);
				}
			...
			if (centerPointer >= chartCenter) {
				minIndex = minIndex < maxIndex ? minIndex = Math.min(maxIndex, minIndex + scaleStep) : minIndex;
			} else if (centerPointer < chartCenter) {
				maxIndex = maxIndex > minIndex ? maxIndex = Math.max(minIndex, maxIndex - scaleStep) : maxIndex;
			}
			zoomNS.zoomCumulativeDelta = 0;
		}
